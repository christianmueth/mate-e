...
export default async function BillingPage({
  ...
  try {
    ...
    console.error("[BillingPage] render failed:", error);
    return renderBillingUnavailable();
  }
}
  ...
  return (
    <main className="mx-auto flex w-full max-w-4xl flex-col gap-6 px-6 py-10">
      <section className="rounded-[2rem] border border-slate-200 bg-gradient-to-br from-slate-950 via-slate-900 to-teal-950 p-7 text-white shadow-[0_24px_90px_rgba(15,23,42,0.24)]">
        ...
        <h1 className="mt-3 text-3xl font-semibold tracking-tight">Mate-E Premium</h1>
        <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-200">
          Billing is temporarily unavailable in this deployment. The webhook endpoint can still receive Stripe events once the environment is fully healthy.
        </p>
      </section>
      ...
      <section className="rounded-[1.75rem] border border-amber-200 bg-amber-50 p-6 shadow-sm">
        <p className="text-sm leading-7 text-amber-950">
          An unexpected server dependency failed while loading billing. Retry in a moment. If the problem persists, verify the production database schema and Stripe environment variables for this deployment.
        </p>
      </section>
    </main>
  );
