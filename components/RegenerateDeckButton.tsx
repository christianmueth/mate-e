...
export default function RegenerateDeckButton({ deckId }: { deckId: string }) {
  ...
  async function onClick() {
    if (busy) return;
    const count = Number(prompt("How many new workspace items should this workspace add? (5-50)", "12"));
    if (!count || isNaN(count)) return;

    setBusy(true);
    const t = toast.loading("Refreshing workspace guidance...");
    try {
      ...
      toast.success(`Added ${json.added} new workspace items`);
      router.refresh();
    } catch (e: any) {
      toast.error(e?.message || "Could not refresh workspace guidance");
    } finally {
