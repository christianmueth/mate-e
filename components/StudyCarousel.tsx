...
import {
  ...
  writeStoredTutorChatSessionContext,
...
export default function StudyCarousel({
  ...
  const loadQueue = useCallback(async () => {
    ...
    try {
      ...
      const qRes = await fetch(studyUrl.toString(), { cache: "no-store" });
      const qJson = qRes.ok ? await safeJson(qRes) : null;

      setQueue(Array.isArray(qJson?.cards) ? qJson.cards : []);
      setIdx(0); setShowBack(false); setAnswerDraft(""); setCoachResult(null); setPolicySummary(null); setSessionEvents([]); setSessionComplete(false);

      if (!qRes.ok) toast.error("We couldn't prepare your workspace copilot session right now.");
    } catch (error: unknown) {
      toast.error(getErrorMessage(error, "We couldn't load this workspace copilot session right now.")); setQueue([]);
  ...
  useEffect(() => {
    ...
    writeStoredTutorChatSessionContext(nextContext);
  ...
  const mark = useCallback(async (rating: "again" | "good" | "easy") => {
    ...
    try {
      ...
      toast.success(rating === "easy" ? "Ready to act recorded." : rating === "good" ? "Iteration state recorded." : "Blocker recorded.");
      if (coachingContext) {
        toast.message(recovered ? "Mate-E recorded that this intervention helped." : "Mate-E recorded that this thread still needs another pass.");
      }

      if (next.length === 0) { setSessionComplete(true); toast.success("Workspace session complete."); }
    } catch (error: unknown) { toast.error(getErrorMessage(error, "We couldn't save that workspace step.")); }
  }, [answerDraft, coachResult, current, idx, queue]);

  async function coachCurrentCard() {
    if (!current) return;
    const studentAnswer = answerDraft.trim();
    if (!studentAnswer) {
      toast.error("Write a short answer first so Mate-E has something to respond to.");
      return;
    }
    ...
    try {
      const res = await fetch("/api/tutoring/guide", {
        ...
        body: JSON.stringify({
          ...
          title: "Inline workspace analysis",
          origin: "study_carousel",
        }),
      });
      const data = (await safeJson(res)) as TutoringGuideResponse | null;
      if (!res.ok || !data?.ok) {
        throw new Error(readApiError(data, "We couldn't prepare analysis for this step."));
      }
      setCoachResult(data);
      ...
      toast.error(getErrorMessage(error, "We couldn't load analysis right now."));
    } finally {
      setCoachLoading(false);
    }
  }
  ...
  }, [current, mark, queue.length]);

  if (loading) return <div className="rounded border p-6 text-sm text-gray-500">Preparing your workspace copilot session...</div>;
  if (!queue.length && !sessionComplete)
    return (
      <div className="rounded border p-6 text-sm text-gray-500 flex items-center justify-between">
        <span>Mate-E does not have an active workspace intelligence thread waiting right now.</span>
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-700">Suggested next step</h3>
  ...
  const card = current!;
  const suggestedActions = buildSuggestedActions(card, coachResult, policySummary);
    ...
    <div className="space-y-4">
      {(focusConcept || focusReason) ? (
        <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4 text-sm text-sky-950">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="font-semibold">Active thread: {focusConcept || "Operational analysis"}</p>
              {focusReason ? <p className="mt-1 text-sky-900">Why Mate-E picked this: {focusReason}</p> : null}
            </div>
            {recommendationSource ? (
      ...
      <div className="rounded-2xl border border-sky-200 bg-gradient-to-r from-sky-50 via-white to-cyan-50 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="max-w-3xl">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-sky-700">Workspace Copilot</p>
      ...
      <div className="rounded-2xl border p-6 space-y-5">
        <div className="text-xs text-gray-500">
          Context item {idx + 1} / {queue.length} • Use <kbd>1</kbd>/<kbd>2</kbd>/<kbd>3</kbd> to classify the outcome of this pass
        </div>
        <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500">Current context</p>
            <h3 className="mt-2 text-lg font-semibold text-slate-950 whitespace-pre-wrap">{card.question}</h3>
            <p className="mt-3 text-sm leading-6 text-slate-700 whitespace-pre-wrap">{card.answer}</p>
          </div>
          <div className="rounded-2xl border border-teal-200 bg-teal-50 p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-teal-700">Suggested next steps</p>
            <div className="mt-3 space-y-2">
              {suggestedActions.map((action) => (
                <div key={action} className="rounded-xl border border-teal-100 bg-white/80 px-3 py-2 text-sm text-teal-950">
                  {action}
                </div>
              ))}
            </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-semibold text-slate-900">Ask Workspace</h3>
            <p className="text-xs text-slate-500">Describe the dependency, blocker, decision, or ambiguity you want Mate-E to analyze next.</p>
          </div>
          <button
          ...
          >
            {coachLoading ? "Analyzing..." : "Run analysis"}
          </button>
        </div>
        <textarea
          value={answerDraft}
          onChange={(event) => setAnswerDraft(event.target.value)}
          placeholder="What would you like to investigate, resolve, or plan from this workspace item?"
        ...
        {coachResult ? (
          <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 space-y-3">
              <div className="flex items-center justify-between gap-3">
                <h4 className="text-sm font-semibold text-emerald-950">AI observations</h4>
            ...
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 space-y-3">
              <h4 className="text-sm font-semibold text-amber-950">Context signals</h4>
              <div className="flex flex-wrap gap-2">
                {(coachResult.misconceptionSignals || []).length ? (
                  ...
                  <span className="text-xs text-amber-900">No strong signal pattern stood out in this input.</span>
                )}
              </div>
              <div className="grid grid-cols-2 gap-3">
                <MiniCoachStat label="Signal confidence" value={formatScore(coachResult.verification?.confidence)} />
                <MiniCoachStat label="Analysis confidence" value={formatScore(coachResult.selectedStrategy?.confidence)} />
              </div>
              {policySummary ? (
                <div className="rounded-xl border border-amber-200 bg-white/80 p-3 text-xs text-amber-900 space-y-1">
                  <div>Most helpful recent intervention here: {policySummary.topStrategy || "none yet"}</div>
                  <div>Typical confidence in this pattern: {formatScore(policySummary.averageConfidence)}</div>
                  <div>Recent unstable examples: {policySummary.lowConfidenceRuns}</div>
                  <div>Your next action helps Mate-E learn whether this intervention actually helped.</div>
                </div>
              ) : null}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button className="px-3 py-1.5 rounded bg-red-600 text-white" onClick={() => mark("again")}>1 · Still blocked</button>
        <button className="px-3 py-1.5 rounded bg-yellow-500 text-white" onClick={() => mark("good")}>2 · Ready to iterate</button>
        <button className="px-3 py-1.5 rounded bg-green-600 text-white" onClick={() => mark("easy")}>3 · Ready to act</button>
      </div>
    </div>
  );
}
...
function buildInlineWhyWon(result: TutoringGuideResponse, policy: StrategyPolicySummary | null) {
  ...
  if (result.selectedStrategy?.label) {
    reasons.push(`Selected ${result.selectedStrategy.label.toLowerCase()} after reranking the available intervention paths.`);
  }
  if (typeof result.verification?.confidence === "number") {
    reasons.push(`Current confidence is ${formatScore(result.verification.confidence)}, so the response is calibrated to that uncertainty level.`);
  }
  if (result.misconceptionSignals?.length) {
    reasons.push(`The system classified this input under ${humanizeMisconceptionCategory(result.misconceptionSignals[0])}, which influenced the intervention choice.`);
  }
...
function buildTutorPresence({
  ...
  if (coachResult) {
    return {
      title: selectedStrategy
        ? `I found a useful intervention: ${selectedStrategy.label.toLowerCase()}.`
        : "I have an analysis move ready for this workspace item.",
      message: misconception
        ? `Your input suggests ${humanizeMisconceptionCategory(misconception).toLowerCase()}, so I am steering this session toward a more targeted intervention instead of a generic response.`
        : "I am using your input and recent workspace state to pick the next intervention rather than showing a generic explanation.",
      badge: selectedStrategy?.strategyType || recommendationSource || null,
      cues: [
        selectedStrategy?.hint || coachResult.tutoring?.final_answer || "Ask for analysis after you describe the blocker or decision you want help with.",
        preferredStyle
          ? `Current explanation style match: ${preferredStyle.toLowerCase()}.`
          : "Mate-E is still learning which explanation style works best for you.",
        lowConfidenceStreak > 0
          ? `Recent confidence has been unstable for ${lowConfidenceStreak} steps, so Mate-E is keeping the pace slower.`
  ...
  return {
    title: focusConcept ? `Today's session centers on ${focusConcept}.` : `Start with ${trimQuestion(card?.question || "this workspace item", 56)}.`,
    message: focusReason
      ? `Why this is next: ${focusReason}`
      : "Describe what you want to investigate. Once you do, Mate-E can adapt the next intervention to your confidence, context pattern, and recent workspace history.",
    badge: recommendationSource ? recommendationSource.replace(/_/g, " ") : null,
    cues: [
      focusConcept
        ? `The queue is already biased toward ${focusConcept} so you spend less time hunting for the right operational thread.`
        : "Start with your own framing. Mate-E becomes more useful when it can react to your context instead of replacing it.",
      "If the thread feels blocked, ask for analysis early so the decision signal is captured.",
      "After a useful intervention, mark the outcome so Mate-E can learn whether it actually helped.",
    ],
  };
}
...
function buildSessionReflection(events: SessionEvent[], deckId: string, focusConcept?: string | null) {
  ...
  const summary = recoveredCount > 0
    ? `The session did more than move items forward. You hit resistance, adjusted with guidance, and recovered enough momentum to make this thread feel more workable than it did at the start.`
    : easyCount > againCount
      ? `You kept this session steady and moved through the material with more control than hesitation, which usually means the thread is starting to settle instead of feeling newly fragile.`
      : `This pass surfaced a real friction point, which is still useful. It shows Mate-E where to slow down and which reasoning path needs to be clearer next time.`;
  ...
  const stillUnstable = topMisconception
    ...
    : againCount > 0
      ? `${focusLabel || "The shakier cards"} still needs another pass before Mate-E should treat it as stable. Right now the pattern looks more like partial recall than durable understanding.`
      : `Nothing fully collapsed this session, but Mate-E should still treat ${focusLabel || "this topic"} as recently improved rather than permanently stable.`;
  ...
  const resumeReason = topMisconception
    ? `${focusLabel || "This concept"} still becomes unstable around ${humanizeMisconceptionCategory(topMisconception).toLowerCase()}, so Mate-E is bringing you back here before that pattern hardens.`
    : topStrategy
      ? `${focusLabel || "This concept"} improved with ${topStrategy.toLowerCase()}, but it still needs one more focused pass before Mate-E should treat it as stable.`
      : againCount > 0
        ? `${focusLabel || "This concept"} was still shaky at the end of the session, so Mate-E is resuming it first instead of letting the thread go cold.`
...
function buildSuggestedActions(
  card: StudyCard,
  coachResult: TutoringGuideResponse | null,
  policy: StrategyPolicySummary | null
) {
  const actions = [] as string[];
  actions.push(`Clarify the open thread around ${trimQuestion(card.question, 72)}.`);
  if (coachResult?.weakTopicMatches?.[0]) {
    actions.push(`Reconnect this item to ${coachResult.weakTopicMatches[0]} before branching into new work.`);
  }
  if (coachResult?.selectedStrategy?.label) {
    actions.push(`Use ${coachResult.selectedStrategy.label.toLowerCase()} to move this workspace item forward.`);
  } else {
    actions.push("Ask Mate-E to surface the most likely blocker, dependency, or next move.");
  }
  if (policy?.topStrategy) {
    actions.push(`Recent runs suggest ${policy.topStrategy.toLowerCase()} is the most reliable intervention here.`);
  }
